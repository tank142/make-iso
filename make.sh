...
mkdir -p "$ROOT"/var/lib/pacman/pkg
RUN "\033[1;36m\033[2mInstall Arch Linux \033[0m" pacman -r "$ROOT" --noconfirm  --config "$DIRSTACK"/pacman.conf --cachedir "$CACHE_DIR" -Sy $PKG
...
RUN "\033[1;36m\033[2mCleaning \033[0m" rm -rfv "$ROOT"/var/cache/pacman/pkg/* "$ROOT"/var/lib/pacman/sync/* "$ROOT"/var/log/* "$ROOT"/tmp/* "$ROOT"/root/.bash_history
...
RUN "" sed -i s:INITRD_IMG:`basename -a "$ROOT_ISO"/boot/*initramfs*fallback* | head -1`: "$ROOT_ISO"/boot/grub/grub.cfg
